(function () {
    var app = angular.module("maps.helper");
    var mapsDirective = function ($compile) {
        return {
            restrict: "E",
            templateUrl: "",
            scope: {
                geoJSON: "=gHDataSource",
                center: "=center",
                activityIcon: "=activityIcon",
                startIcon: "=startIcon",
                onLoadActivity: "&ghOnActivitySelect",
                ghFilterDetails: "=",
                ghMapFilters: "=",
                showRoute: "@"
            },
            link: function (scope, el, attr) {
                ...
                scope.$watch("ghFilterDetails",
                    function (newValue, oldValue) {
                        if (newValue !== oldValue) {
                            //Show filter details On Map
                            drawFilters();
                        }
                    });
            },
            controller: function ($scope) {
                ...
                var initMap = function () {
                    ...
                    //addGeoJSONDataLayer();
                    if ($scope.showRoute == "true" && $scope.geoJSON.LatLong) {
                        destination = "" + $scope.geoJSON.LatLong.split(",")[0] + "," + $scope.geoJSON.LatLong.split(",")[1] + "";
                        calcRoute();
                    }
                    else if ($scope.geoJSON.LatLong) {
                        createGeoJSON();
                    }
                }
					            ...
					            var contentString = '<div id="content' + centerMarker.id + '" class="map-content">' +
                                                         '<div class="column-12">' +
                                                                 "<div>" +
                                                                     "<span>" + item.Name + "</span>" +
                                                                 "</div>" +
                                                                 '<div ng-click="loadChildCareDetails(boundItem)">' +
                ...
                $scope.drawFilters = function () {
                    $scope.ghMapFilters = true;
                }
    ...
    app.directive("ghMaps", mapsDirective);
}());